...
The \pkg{autoplotly} package calls \pkg{ggfortify}'s \code{autoplot()} method that invokes an registered S3 generic functions \footnote{\url{http://adv-r.had.co.nz/S3.html}} for the applied object to create the visualizations with \pkg{pplot2} style. Next, the generated \code{ggplot} object is translated to \code{plotly} object with interactive graphical components leveraging \code{plotly::ggplotly}. Additional clean-up and correction for unneccessary and corrupted components are then performed. For example, if we want to generate interactive visualization for principal components analysis results produced from \code{prcomp(...)}, the following will be executed in order:
...
\code{`+` <- function(e1, e2)} operator is applied to a \code{ggplot} element as the second argument \code{e2}, e.g. \code{ggplot2::ggtitle(...)}, the \code{ggplot} object that we attached to the output of \code{autoplotly} earlier will be used as the first argument \code{e1}, borrowing \pkg{ggplot2}'s extensibility. Similarly, if we are adding \pkg{plotly} interactive components, the \code{plotly} object from the output \code{p} will be used instead.
